...
public class Homework1 {
    ...
    public void streamPipeline8() {
        countries.stream().sorted(Comparator.comparingInt(country -> country.getTimezones().size())).forEach(s -> System.out.println(s.getName() + " : " + s.getTimezones().size()));
    }
...
ed by the language code "es").
    ...
    public void streamPipeline10() {
        countries.stream().filter(t -> t.getArea() == null).map(Country::getName).forEach(System.out::println);
    }
    ...
    public void streamPipeline11() {
        countries.stream().map(country -> country.getTranslations()).flatMap(translation -> translation.keySet().stream()).distinct().sorted().forEach(System.out::println);
    }
    ...
    public void streamPipeline15() {
        countries.stream().filter(country -> country.getArea() != null && country.getArea().compareTo(new BigDecimal(1)) == -1 ).map(Country::getName).forEach(System.out::println);
    }
